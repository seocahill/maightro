...
class Option1aTest < Test::Unit::TestCase
  ...
  def test_block_timing
    ...
    assert_equal Option1a.new.stops('Ballina', 'Manulla Junction', dep).max_by { |t| t[1] }[1], mal_arr

    bal_arr = Time.parse('12:27')
    assert_equal Option1a.new.stops('Manulla Junction', 'Ballina', dep).max_by { |t| t[1] }[1], bal_arr

    man_arr = Time.parse('12:19')
    assert_equal Option1a.new.stops('Westport', 'Manulla Junction', dep).max_by { |t| t[1] }[1], man_arr
    wes_arr = Time.parse('12:23')
    assert_equal Option1a.new.stops('Manulla Junction', 'Westport', dep).max_by { |t| t[1] }[1], wes_arr
  end
