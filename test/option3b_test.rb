...
require_relative '../models/scenarios/option_3b'
require 'test/unit'
...
class Option3bTest < Test::Unit::TestCase
  ...
  def test_min_dwell_local_trains
    ...
    rows = (@covey + @covey_return).select { |t| t[-1] =~ /LC/ }.sort_by { |r| r[2] }
    assert_equal rows.each_cons(2).map { |s, e| (Time.parse(e[2]) - Time.parse(s[3])).fdiv(60) }.min, 3.0

    # busiest block is Castlebar - Westport
    rows = (Option3b.new('20221222', 'Westport',
                         'Castlebar').rows + Option3b.new('20221222', 'Castlebar', 'Westport').rows).select do |t|
             t[-1] =~ /LC/
           end.sort_by do |r|
      r[2]
    end
